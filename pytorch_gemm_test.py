...
for gemm in gemm_list:
    ...
    trial_list = []
    # with xp.Trace(f"b={b},m={m},k={k},n={n}"):
    for i in range(n_trials):
      A = torch.rand((b, m, k), dtype=torch.float32, requires_grad=False)
      B = torch.rand((b, k, n), dtype=torch.float32, requires_grad=False)
      A.to(device)
      B.to(device)
    ...
    print(f"{sum(result_dict[gemm][3:])/len(result_dict[gemm][3:])*1000:.3f} ms")
