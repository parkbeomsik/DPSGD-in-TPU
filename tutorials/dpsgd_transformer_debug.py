...
def load_data():
  ...
  train_data = {"input_ids": np.random.randint(0, 28996, size=(FLAGS.batch_size*FLAGS.n_batches, FLAGS.seq_length)),
...
def main(unused_argv):
  ...
  def get_per_sample_gradients(arg):
    with tf.keras.backend.name_scope("computing_grads"):
      with tf.GradientTape() as g:
        inp, label = arg
        inp["input_ids"] = tf.expand_dims(inp["input_ids"], 0)
        inp["attention_masks"] = tf.expand_dims(inp["attention_masks"], 0)
        inp["token_type_ids"] = tf.expand_dims(inp["token_type_ids"], 0)
        inp["position_ids"] = tf.expand_dims(inp["position_ids"], 0)
  ...
  def get_private_grads(x_batch_train, y_batch_train, step):
    ...
    return per_sample_grads, final_gradients
  ...
  for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
    ...
    per_sample_grads, final_gradients = strategy.run(get_private_grads, (x_batch_train, y_batch_train, step))
