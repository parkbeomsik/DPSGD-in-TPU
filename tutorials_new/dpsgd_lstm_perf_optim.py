...
def load_data():
  ...
  train_data = np.random.rand(FLAGS.batch_size*FLAGS.n_batches, FLAGS.seq_length, 258)
...
def main(unused_argv):
  ...
  with strategy.scope():
    ...
    for i in range(FLAGS.n_lstm_layers):
      if i == 0:
        batch_input_shape = (FLAGS.seq_length, FLAGS.batch_size, FLAGS.embedding_size)
        
      if i == FLAGS.n_lstm_layers - 1:
        if not (i == 0):
          batch_input_shape = (FLAGS.seq_length, FLAGS.batch_size, FLAGS.hidden_size)
        return_sequences = False
      else:
        return_sequences = True
        
      model.add(tf.keras.layers.LSTM(FLAGS.hidden_size, input_length=FLAGS.seq_length, batch_input_shape=batch_input_shape, return_sequences=return_sequences, time_major=True)) # , time_major=True unroll=True
