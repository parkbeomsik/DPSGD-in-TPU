...
def main(unused_argv):
  ...
  logs = "logs/" + FLAGS.model + "_" + str(FLAGS.seq_length) + "_" + ("Reweight" if FLAGS.dpsgd else "SGD") + "_" + datetime.now().strftime("%Y%m%d-%H%M%S")
  ...
  def get_private_grads(x_batch_train, y_batch_train, step):
    ...
    div = tf.vectorized_map(compute_clipping_factor, per_sample_grads)
    print(div)
    summed_gradients = get_per_batch_gradients((x_batch_train, y_batch_train, div))
